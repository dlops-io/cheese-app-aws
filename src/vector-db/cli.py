...
CHROMADB_HOST = os.environ["CHROMADB_HOST"].strip()
CHROMADB_PORT = os.environ["CHROMADB_PORT"].strip()
EMBEDDING_DIMENSION = 1536  # Claude/Titan embeddings are 1536 dimensions
...
# Update the generate_query_embedding function
def generate_query_embedding(query):
    embedding = embedding_model.embed_query(query)
    return embedding

# Update the generate_text_embeddings function
def generate_text_embeddings(chunks, dimensionality: int = 1536, batch_size=250):
    all_embeddings = []
    for chunk in chunks:
...
def validate_aws_bedrock_access():
    """
    Validates AWS credentials and Bedrock access by:
    1. Checking if credentials are configured
    2. Checking if Bedrock service is accessible
    3. Verifying access to required models
    """
    try:
        ...
        bedrock_mgmt = boto3.client('bedrock', 
								   ...
								   aws_access_key_id=AWS_ACCESS_KEY_ID, 
								   aws_secret_access_key=AWS_SECRET_ACCESS_KEY)
        # Test listing models (validates basic access)
        models = bedrock_mgmt.list_foundation_models()

        
        # Check for specific required models
        required_models = {
            'amazon.titan-embed-text-v1': False,  # for embeddings
            'anthropic.claude-3-5-sonnet-20240620-v1:0': False  # for chat
        }
        
        for model in models['modelSummaries']:
            if model['modelId'] in required_models:
                required_models[model['modelId']] = True
        
        # Check if any required models are missing
        missing_models = [model for model, found in required_models.items() if not found]
        if missing_models:
            print("⚠️  Warning: The following required models are not enabled:")
            for model in missing_models:
                print(f"   - {model}")
            return False
        
        print("✅ AWS Bedrock access validated successfully!")
        return True

    except ClientError as e:
        error_code = e.response['Error']['Code']
        error_message = e.response['Error']['Message']
        print(f"❌ AWS Bedrock access validation failed!")
        ...
        if error_code == 'InvalidSignatureException':
            ...
            print("2. Credentials not set in environment or ~/.aws/credentials")
            print("3. Clock skew between your system and AWS")
...
def main(args=None):
	...
	# if not validate_aws_bedrock_access():
	# 	print("\nPlease fix AWS access issues before continuing.")
	# 	return
